...
RegisterServerEvent('nova-admin:addvariable')
AddEventHandler('nova-admin:addvariable', function(type, value)
  client_var[source] = {}
...
GetWarn = function(xPlayer)
  ...
  local result = MySQL.Sync.fetchAll("SELECT * FROM nova_admin WHERE identifier = @identifier", {
...
GetStaffBan = function(xPlayer)
  local identifier = GetLicense(xPlayer.source, "license")
  CheckSql(xPlayer.source)
  local result = MySQL.Sync.fetchAll("SELECT * FROM nova_admin")
  local bans = {}
  for i=1, #result, 1 do 
...
ESX.RegisterServerCallback('nova-admin:getPlayers', function(source, cb)
  cb(getPlayers())
end)

ESX.RegisterServerCallback('nova-admin:getData', function(source, cb)
  local data = {}
...
ESX.RegisterServerCallback('nova-admin:getInfoStaff', function(source, cb)
  local xPlayer = ESX.GetPlayerFromId(source)
  local rankLabel = Config.AdminGroup[xPlayer.getGroup()]
  if rankLabel == nil then 
    rankLabel = Config.Lang[Config.Language]['utente']
...
ESX.RegisterServerCallback('nova-admin:sonoStaff', function(source, cb)
...
CheckSql = function(id)
  ...
  if result[1] == nil then 
    MySQL.Async.execute("INSERT INTO nova_admin (identifier) VALUES (@identifier)", {
      ['@identifier'] = GetLicense(xPlayer.source, "license"),
    })
    Wait(1000)
  end
...
RegisterServerEvent('nova-admin:action')
AddEventHandler('nova-admin:action', function(data)
  ...
  if data.action == 'warn' then 
    ...
    if target then 
      ...
      MySQL.Async.execute("UPDATE nova_admin SET warn = @warn WHERE identifier = @identifier", {
  ...
  elseif data.action == 'kick' then
    ...
    if target then 
      ...
      MySQL.Async.execute("UPDATE nova_admin SET kick = @kick WHERE identifier = @identifier", {
  ...
  MySQL.Async.execute("UPDATE nova_admin SET ban = @ban WHERE identifier = @identifier", {
  ...
  elseif data.action == 'adminjail' then
    ...
    if target then 
      ...
      MySQL.Sync.execute("UPDATE nova_admin SET jailCoords = @jailCoords WHERE identifier = @identifier", {
        ['@identifier'] = GetLicense(target.source, "license"),
      ...
      MySQL.Async.execute("UPDATE nova_admin SET jail = @jail WHERE identifier = @identifier", {
        ['@jail'] = json.encode(myJail),
        ['@identifier'] = GetLicense(target.source, "license"),
      })

        MySQL.Sync.execute("UPDATE nova_admin SET inJail = @inJail WHERE identifier = @identifier", {
          ['@identifier'] = GetLicense(target.source, "license"),
          ['@inJail'] = 1
        })

      TriggerClientEvent('nova-admin:adminjail', target.source)
      local fields = {
        {
          name = Config.Lang[Config.Language]['nome_staff'],
          value = GetPlayerName(staff.source),
  ...
  elseif data.action == 'setped' then
    local ped = value1
    if target then 
      TriggerClientEvent('nova-admin:setped', target.source, ped)
    end
  elseif data.action == 'dm' then
    local msg = value1
    if target then 
      TriggerClientEvent('nova-admin:dm', target.source, msg)
      local fields = {
        {
          name = Config.Lang[Config.Language]['nome_staff'],
          value = GetPlayerName(staff.source),
  ...
  elseif data.action == 'annuncio' then 
    TriggerClientEvent('nova-admin:announce', -1, value1)
    local fields = {
      {
        name = Config.Lang[Config.Language]['nome_staff'],
        value = GetPlayerName(staff.source),
  ...
  elseif data.action == "cleararea" then 
    local radius = value1
    TriggerClientEvent('nova-admin:cleararea', -1, radius)
    local fields = {
      {
        name = Config.Lang[Config.Language]['nome_staff'],
        value = GetPlayerName(staff.source),
  ...
  TriggerClientEvent('nova-admin:updatePlayers', -1)
end)
...
RegisterServerEvent('nova-admin:skinmenu')
AddEventHandler('nova-admin:skinmenu', function(id)
  local src = source
...
RegisterServerEvent('nova-admin:heal')
AddEventHandler('nova-admin:heal', function(id)
  local src = source
  local staff = ESX.GetPlayerFromId(src)
  local target = ESX.GetPlayerFromId(id)
  local fields = {
...
RegisterServerEvent('nova-admin:revive')
AddEventHandler('nova-admin:revive', function(id)
  local src = source
  local staff = ESX.GetPlayerFromId(src)
...
RegisterServerEvent('nova-admin:kill')
AddEventHandler('nova-admin:kill', function(id)
  local src = source
  local staff = ESX.GetPlayerFromId(src)
  ...
  TriggerClientEvent('nova-admin:kill', id)
end)


RegisterServerEvent('nova-admin:goto')
AddEventHandler('nova-admin:goto', function(id)
  local src = source
  local staff = ESX.GetPlayerFromId(src)
  local target = ESX.GetPlayerFromId(id)
  if target == nil then 
...
RegisterServerEvent('nova-admin:bring')
AddEventHandler('nova-admin:bring', function(id)
  local src = source
  local staff = ESX.GetPlayerFromId(src)
  local target = ESX.GetPlayerFromId(id)
  if target == nil then 
...
RegisterServerEvent('nova-admin:clearInv')
AddEventHandler('nova-admin:clearInv', function(id)
  local src = source
  local staff = ESX.GetPlayerFromId(src)
...
ESX.RegisterServerCallback('nova-admin:getInJail', function(source, cb)
  cb(InJail(source))
end)

RegisterServerEvent('nova-admin:unjail')
AddEventHandler('nova-admin:unjail', function(id)
  local src = source
  local staff = ESX.GetPlayerFromId(src)
  ...
  TriggerClientEvent('nova-admin:updatePlayers', -1)
  TriggerClientEvent('nova-admin:unjail', target.source)

end)


RegisterServerEvent('nova-admin:wipe')
AddEventHandler('nova-admin:wipe', function(id)
  local src = source
  local staff = ESX.GetPlayerFromId(src)
  local target = ESX.GetPlayerFromId(id)
  if target == nil then 
...
RegisterServerEvent('nova-admin:deletewarn')
AddEventHandler('nova-admin:deletewarn', function(id, index)
  local src = source
  local staff = ESX.GetPlayerFromId(src)
...
ESX.RegisterServerCallback('nova-admin:getOrario', function(source, cb)
...
ESX.RegisterServerCallback('nova-admin:getCoordsPlayer', function(source,cb,id)
  local xPlayer = ESX.GetPlayerFromId(id)
  if xPlayer == nil then return end
  cb(xPlayer.getCoords(true), xPlayer.getCoords(false))
end)

RegisterServerEvent('nova-admin:freeze')
AddEventHandler('nova-admin:freeze', function(id)
  local src = source
  local staff = ESX.GetPlayerFromId(src)
  ...
  TriggerClientEvent('nova-admin:freeze', target.source)
...
RegisterServerEvent('nova-admin:sfreeze')
AddEventHandler('nova-admin:sfreeze', function(id)
  local src = source
  local staff = ESX.GetPlayerFromId(src)
  ...
  TriggerClientEvent('nova-admin:sfreeze', target.source)
  if client_var[target.source].freeze == 1 then 
    client_var[target.source].freeze = false
  else
    client_var[target.source].freeze = 1
...
RegisterServerEvent('nova-admin:revocaban')
AddEventHandler('nova-admin:revocaban', function(staffId, id)
  UnBan(id)
  TriggerClientEvent('nova-admin:updatePlayers', -1)
...
RegisterServerEvent('nova-admin:reviveall')
AddEventHandler('nova-admin:reviveall', function()
...
ESX.RegisterServerCallback('nova-admin:getAllKicks', function(source, cb)
  local result = MySQL.Sync.fetchAll("SELECT * FROM nova_admin")
  local counterKicks = 0
  for i=1, #result, 1 do 
    local kick = json.decode(result[i].kick)
...
RegisterServerEvent('nova-admin:clearped')
AddEventHandler('nova-admin:clearped', function(id)
  local src = source
  local staff = ESX.GetPlayerFromId(src)
  ...
  TriggerClientEvent('nova-admin:clearped', target.source)
end)
...
RegisterServerEvent('nova-admin:webhook')
AddEventHandler('nova-admin:webhook', function(title, fields, webhook)
