...
Citizen.CreateThread(function()
    ...
    local jail = nil 
    ESX.TriggerServerCallback('nova-admin:getInJail', function(jail1) 
        jail = jail1
    end)
    while jail == nil do
        Citizen.Wait(0)
...
if Config.Keybinds.nomitesta.enable then 
    ...
    RegisterCommand('-+nomitesta', function(source, args, rawCommand)
        ...
        TriggerServerEvent('nova-admin:webhook', Config.Lang[Config.Language]["nomitesta"], fields, "nomiplayer")
    end)
end
...
RegisterNUICallback('deletewarn', function(data)
    ...
    TriggerServerEvent('nova-admin:deletewarn', id, index)
end)

SonoStaff = function()
    local staff1 = nil
   ESX.TriggerServerCallback('nova-admin:sonoStaff', function(staff) 
        staff1 = staff
    end)
    while staff1 == nil do
        Citizen.Wait(0)
...
CreateThread(function()
    while true do
        ESX.TriggerServerCallback('nova-admin:getPlayers', function(t)
            players = t
        end)
        postMessage({
            type = "UPDATE_PLAYERS",
...
OpenAdminMenu = function()
    ESX.TriggerServerCallback('nova-admin:getPlayers', function(t)
        ESX.TriggerServerCallback('nova-admin:getData', function(data)
            if SonoStaff() == false then return end
...
RegisterNUICallback('skinmenu', function(data, cb)
    local id = data.id
    TriggerServerEvent('nova-admin:skinmenu', id)
end)
...
RegisterNUICallback('spectate', function(data)
    ...
    TriggerServerEvent('nova-admin:webhook', Config.Lang[Config.Language]["inizio_spectate"], fields, "spectate")
end)

RegisterNetEvent('nova-admin:dm')
AddEventHandler('nova-admin:dm', function(text)
    SendDMMessage(text)
end)
...
RegisterNUICallback('repairvehicle', function()
    ...
    if veh ~= 0 then
        ...
        TriggerServerEvent('nova-admin:webhook', Config.Lang[Config.Language]["ripara_veicolo"], fields, "repairvehicle")
        ESX.ShowNotification(Config.Lang[Config.Language]["repair_vehicle2"])
    end
end)

RegisterNetEvent('nova-admin:cleararea')
AddEventHandler('nova-admin:cleararea', function(radius)
    ...
    TriggerServerEvent('nova-admin:webhook', Config.Lang[Config.Language]["clear_area"], fields, "cleararea")
    ESX.ShowNotification(Config.Lang[Config.Language]["clear_area2"])
end)

RegisterNUICallback('clearped', function(data)
    local id = data.id
    TriggerServerEvent('nova-admin:clearped', id)
    ESX.ShowNotification(Config.Lang[Config.Language]["clear_ped2"])
end)

RegisterNetEvent('nova-admin:clearped')
AddEventHandler('nova-admin:clearped', function()
    local ped = PlayerPedId()
  ...
  while true do
    if spectating then 
        ...
        if IsControlJustPressed(0, 38) then
            ...
            TriggerServerEvent('nova-admin:webhook', Config.Lang[Config.Language]["fine_spectate"], fields, "spectate")
        end
        ESX.TriggerServerCallback('nova-admin:getCoordsPlayer', function(coords, coords2) 
...
RegisterNUICallback('unjail', function(data, cb)
    local id = data.id
    TriggerServerEvent('nova-admin:unjail', id)
end)
...
RegisterCommand('noclip', function(source, args, rawCommand)
    ...
    if noclipEnabled then 
        ...
        TriggerServerEvent('nova-admin:webhook', 'NoClip', fields, "noclip")
      else
        ...
        TriggerServerEvent('nova-admin:webhook', Config.Lang[Config.Language]["noclip"], fields, "noclip")
      end
end)
...
RegisterNUICallback('revocaban', function(data)
    local id = data.id
    TriggerServerEvent('nova-admin:revocaban', GetPlayerServerId(PlayerId()), id)
    ESX.ShowNotification(Config.Lang[Config.Language]['revoca_ban'])
end)

RegisterNUICallback('kill', function(data, cb)
    local id = data.id
    TriggerServerEvent('nova-admin:kill', id)
    ESX.ShowNotification(Config.Lang[Config.Language]['kill'])
end)

RegisterNUICallback('goto', function(data, cb)
    local id = data.id
    TriggerServerEvent('nova-admin:goto', id)
end)

RegisterNUICallback('bring', function(data, cb)
    local id = data.id
    TriggerServerEvent('nova-admin:bring', id)
end)

RegisterNUICallback('wipe', function(data, cb)
    local id = data.id
    TriggerServerEvent('nova-admin:wipe', id)
    ESX.ShowNotification(Config.Lang[Config.Language]['wipe'])
end)

RegisterNUICallback('clearinv', function(data, cb)
    local id = data.id
    TriggerServerEvent('nova-admin:clearInv', id)
    ESX.ShowNotification(Config.Lang[Config.Language]['clear_inv2'])
end)

RegisterNetEvent('nova-admin:kill')
AddEventHandler('nova-admin:kill', function()
  local ped = PlayerPedId()
  SetEntityHealth(ped, 0)
end)

RegisterNUICallback('action', function(data, cb)
    TriggerServerEvent('nova-admin:action', data)
end)

RegisterNetEvent('nova-admin:updatePlayers')
AddEventHandler('nova-admin:updatePlayers', function()
    ESX.TriggerServerCallback('nova-admin:getPlayers', function(players)
        ESX.TriggerServerCallback('nova-admin:getData', function(data) 
            if SonoStaff() == false then return end
            postMessage({
                type = "UPDATE_PLAYERS",
...
RegisterNetEvent('nova-admin:setped')
AddEventHandler('nova-admin:setped', function(model)
...
RegisterNetEvent('nova-admin:resetped')
AddEventHandler('nova-admin:resetped', function()
...
RegisterNetEvent('nova-admin:adminjail')
AddEventHandler('nova-admin:adminjail', function()
    inJail = true
    setJailCoords = true
end)
...
RegisterNetEvent('nova-admin:unjail')
AddEventHandler('nova-admin:unjail', function()
  local ped = PlayerPedId()
  setJailCoords = false
  inJail = false
  Wait(1000)
...
RegisterNUICallback('freeze', function(data)
    ...
    TriggerServerEvent('nova-admin:freeze', id)
end)

RegisterNUICallback('sfreeze', function(data)
    local id = tonumber(data.id)
    TriggerServerEvent('nova-admin:sfreeze', id)
end)

RegisterNetEvent('nova-admin:freeze')
AddEventHandler('nova-admin:freeze', function()
    FreezeEntityPosition(PlayerPedId(), true)
end)

RegisterNetEvent('nova-admin:sfreeze')
AddEventHandler('nova-admin:sfreeze', function()
...
RegisterNUICallback('invisibilita', function()
    ...
    TriggerServerEvent('nova-admin:webhook', Config.Lang[Config.Language]["invisibilita"], fields, "invisibilita")
end)
...
RegisterNUICallback('godmode', function()
    ...
    TriggerServerEvent('nova-admin:webhook', "Godmode", fields, "godmode")
    SetNuiFocus(false, false)
end)

RegisterNUICallback('reviveall', function(data, cb)
    SetNuiFocus(false, false)
    TriggerServerEvent('nova-admin:reviveall')
    ESX.ShowNotification(Config.Lang[Config.Language]['revive_all'])
end)

RegisterNetEvent('nova-admin:announce')
AddEventHandler('nova-admin:announce', function(text)
  Announce(text)
end)
...
Citizen.CreateThread(function()
  while true do
    Citizen.Wait(1000)
    TriggerServerEvent('nova-admin:addvariable', 'freeze', IsEntityPositionFrozen(PlayerPedId()))
    ESX.TriggerServerCallback('nova-admin:getOrario', function(orario)
        postMessage({
            type = "SET_ORARIO",
            orario = orario
        })
